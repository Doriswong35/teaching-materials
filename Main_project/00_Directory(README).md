...
There are two possible pathways through which warming could influence discharge: by inducing a change on the maximum root zone storage capacity ($Su_{max}$), and by influencing evapotranspiration.  

!$Su_{max}$: 

Ma et al (2024) found that long-term warming significantly decreased total SOC by 10.6%, also lowering total porosity. Knowing that SOC enhances field capacity, we wanted to know whether rising temperatures would increase the root zone storage capacity ($Su_{max}$), increasing runoff.  

The analysis (running the HBV model) is done in the notebook `HBV_varied_Su_max.ipynb`, using the script `hbv_bmi_project.py`. Results of the MC calibration are stored in `results_mc_slow.csv`. To test the sensitivity of $Su_{max}$ on discharge, we also ran the conventional (temperature-independent) HBV model: `HBV_constant_Su_max.ipynb` using `hbv_bmi_constant.py`.  

For analysis on actual evapotranspiration:   

Temperature, precipitation and evaporation data were used to analyse the relationship between warming and actual evapotranspiration (AET) in the Moselle catchment. For the AET pathway, we regressed the evaporative fraction anomaly (Ea/P) against temperature anomalies to see if a warmer-than-normal month leads to a higher-than-normal evaporative fraction. We also checked how the Temperature and Ea/P relationship is affected by seasons.  

The analysis is carried out in `AET_temperature_analysis.ipynb`  
