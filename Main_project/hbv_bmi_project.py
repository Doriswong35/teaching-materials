...
###############################################################################    
    ...
    def update(self) -> None:
            ...
            if self.current_timestep < self.end_timestep:
                ...
                # Temperature logic
                T_current = self.T.isel(time=self.current_timestep).values
                # T_baseline should be set during initialize()
                self.DelT = T_current - self.T_baseline 
                
                # Dynamic Su_max: Gamma is par[8]
                # Using (1 - Gamma * DelT) means higher T -> lower Su_max
                self.Su_max = self.Su_max0 * (1 - self.Gamma * self.DelT)
